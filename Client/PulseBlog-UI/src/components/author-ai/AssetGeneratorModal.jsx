...
const AssetGeneratorModal = ({ open, assets, onClose }) => {
  ...
  const tabs = [
    ...
    { id: "linkedin", label: "LinkedIn", show: Boolean(assets.linkedin) },
    { id: "twitter", label: "Twitter / X", show: Boolean(assets.twitter) },
    { id: "instagram", label: "Instagram", show: Boolean(assets.instagram) },
    { id: "youtube", label: "YouTube", show: Boolean(assets.youtube) },
    { id: "seo", label: "SEO", show: Boolean(assets.seo) },
    { id: "hooks", label: "Hooks", show: Boolean(assets.hooks) },
  ].filter((tab) => tab.id === "all" || tab.show);

  return (
    <div
      onClick={onClose}
      className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50 overflow-y-auto animate-fadeIn"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="bg-white w-full max-w-4xl max-h-[85vh] flex flex-col rounded-3xl shadow-2xl overflow-hidden border border-gray-100 my-auto"
      >
        {/* Modal Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium">
                Review, copy, and export your generated content
              </p>
            </div>
          </div>
        ...
        <div className="p-8 overflow-y-auto space-y-6 bg-gray-50/30 flex-1">
          ...
          {assets.instagram && (activeTab === "all" || activeTab === "instagram") && (
            ...
            >
              ...
              <TagList
                tags={assets.instagram.hashtags}
                bg="bg-pink-50"
                text="text-pink-600"
              />
            </AssetCard>
            ...
            >
              <div className="space-y-4 text-sm">
                ...
                <div>
                  ...
                  <p className="text-indigo-600 font-mono bg-indigo-50/50 px-2.5 py-1 rounded-lg mt-0.5 inline-block">
                    {assets.seo.slug}
                  </p>
                </div>
                <TagList
                  tags={assets.seo.keywords}
                  bg="bg-emerald-50"
          ...
          {assets.hooks && (activeTab === "all" || activeTab === "hooks") && (
            <AssetCard
              ...
              onCopy={() =>
                handleCopy(
                  "hooks",
                  Array.isArray(assets.hooks) ? assets.hooks.join("\n\n") : ""
                )
              }
              ...
              <div className="space-y-3">
                {Array.isArray(assets.hooks) && assets.hooks.length > 0 ? (
                  assets.hooks.map((hook, index) => (
                    <div
                      key={index}
                      className="bg-amber-50/60 border border-amber-100/60 rounded-2xl p-4 text-gray-800 font-medium flex items-start gap-3"
                    >
                      <span className="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-0.5 rounded-full mt-0.5 shrink-0">
                        #{index + 1}
                      </span>
                  ...
                  <p className="text-gray-500 italic text-sm">No hooks generated.</p>
                )}
              </div>
            </AssetCard>
          )}
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-4">
      {tags.map((tag, index) => (
        <span
          key={index}
          className={`${bg} ${text} px-3 py-1 rounded-full text-xs font-medium`}
        >
          {tag}
        </span>
      ))}
