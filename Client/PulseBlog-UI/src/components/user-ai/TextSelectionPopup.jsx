...
const TextSelectionPopup = ({ visible, position, onAskAI, loading }) => {
  ...
  return (
    <div
      ...
      style={{
        left: `${Math.min(Math.max(position.x, 70), window.innerWidth - 70)}px`,
        top: `${Math.max(position.y, 60)}px`,
      ...
      >
        {loading ? (
          <>
            ...
            <span className="text-[11px] sm:text-xs">Thinking...</span>
          </>
        ) : (
          <>
            ...
            <span className="text-[11px] sm:text-xs">Ask AI</span>
          </>
        )}
      </button>
    </div>
